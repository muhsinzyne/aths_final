...
<head>
    <meta charset="utf-8"/>
    <title> {{ ucfirst(($page['title'] ?? ($page['organization'] ?? '' ))) }} | {{$page['app_name'] ?? ''}}</title>
    ...
    @yield('styles')
    @notifyCss
    <link href="{{ URL::asset('/dist/custom.css')}}" rel="stylesheet" type="text/css" />
...
<script src="{{ URL::asset('libs/custom/custom-scripts.js')}}"></script>
{{-- begin::Javascript --}}
